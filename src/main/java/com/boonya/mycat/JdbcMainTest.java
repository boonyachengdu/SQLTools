...
public class JdbcMainTest {
	...
	public long getMaxId(Connection conn) throws SQLException{
		...
		ResultSet res=st.executeQuery("select max(f_id) from t_location;");
		res.next();
...
class JdbcSHThread implements Runnable{
	...
	public JdbcSHThread(int threadId,Connection conn,String sql,String querySql,String type) {
		super();
	...
	public void run() {
		new JdbcMainTest().insertOrQuery(threadId, conn, sql,querySql,type);
	}
